...
and running the review sequence are 5.8 and 5.9, riding with the step that
merges last.
...
Steps 1 and 5 re-verify those rather than assuming them.
...
## 3. What refuses a constant before it divides
...
- [ ] 3.1 Write the failing cases first: a statistic holding no row, and one
      holding exactly one, fail rather than dividing by `N − 1` of zero, and
      name which statistic was short; a row carrying `matchCount` of 0 fails
      rather than dividing by it.
- [ ] 3.2 Reach the empty case the way the pipeline reaches it, not with a
...
- [ ] 3.3 Refuse before every denominator, not after `var_true`. `p_i` and
...
- [ ] 4.2 Extend the contract's third level to **both** roots (ZOMBIES 19,
      ...
      each.
- [ ] 4.3 Regenerate `src/fixtures/snapshot.json` and say in the pull request
      which branch each root carries.

## 5. The eighth component
...
- [ ] 5.1 Write the failing cases first (ZOMBIES 23, 24, 25, 26, 27): an
...
- [ ] 5.2 Sum over the allies already picked, never the entered enemies, and
...
- [ ] 5.3 Do not weight through `laneWeights` (ZOMBIES 30), for the reason
...
- [ ] 5.4 Re-verify the carried *Empty draft components* scenario rather than
...
- [ ] 5.5 Add a case for two bundles alike but for one `laneAllies` value
...
- [ ] 5.6 Re-run Stryker (ZOMBIES 32). Its floor is scoped to `src/model.ts`,
      which `laning-phase-model`'s step 9 splits; this adds to whatever that
      left, and re-measures the file cap besides.
- [ ] 5.7 Record the suggestion block before and after on one real draft, and
