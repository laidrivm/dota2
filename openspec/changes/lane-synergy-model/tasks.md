...
The four `MODIFIED` deltas carry **eighteen** criteria this change does not
close — every scenario `laning-phase-model` leaves behind that this change
neither renames nor supersedes:
...
each widening from one lane statistic to either — and all four are **closed**
here rather than carried, in steps 2 and 3. A renamed criterion is a new
identifier that no existing test cites, so carrying it would leave it covered
by nothing; the work it names is re-pointing the cases that used the old
slug. The two deltas that rename them carry the before-and-after, so no
citation of an old slug goes quietly unresolved.
...
Steps 1 and 6 re-verify those rather than assuming them.
...
## 2. Centring each statistic against its own hero

Closes `snapshot-build/the-mean-other-hero-gives-no-lane-advantage`,
`snapshot-build/a-hero-with-one-row-in-a-statistic`.

Both are `laning-phase-model`'s scenarios renamed, widened from one lane
statistic to either — so the behaviour is settled and the identifier is not.
A renamed criterion is a **new** identifier that no test cites, which is why
it is closed here rather than carried: the work is re-pointing the cases that
named the old slug, and proving they still hold over two statistics.

- [ ] 2.1 Re-point the predecessor's centring cases at the new identifiers,
      and widen each to run over the ally statistic as well as the opponent
      one. A case left citing `the-mean-opponent-gives-no-lane-advantage`
      resolves to nothing once this syncs.
- [ ] 2.2 Assert the mean over a hero's allies at a position is 0, on the
      same terms the opponent half already asserts over its opponents.
- [ ] 2.3 Assert a cell with exactly one ally centres it to 0, the mean being
      its own value — the pass erasing a one-row cell rather than preserving
      it, which is the arithmetic and not a case to work around.

## 3. A constant for each statistic
...
- [ ] 3.1 Write the failing cases first (ZOMBIES 8, 10, 11, 14): ally rows
      ...
      "the lane constant".
- [ ] 3.2 Derive each over its own rows and never the other's. The two land
      ...
      shared constant.
- [ ] 3.3 Add a case carrying draws and one carrying none (ZOMBIES 12): with
...
- [ ] 3.4 Record both constants with their per-cell ranges, and compare the
      ally one in the pull request against the 29 measured (ZOMBIES 15, 13).

## 4. What refuses a constant before it divides

Closes `snapshot-build/a-statistic-with-fewer-than-two-rows`,
`snapshot-build/a-row-counting-no-match`.

Its own step rather than a third bullet in step 2, and not only because that
would put four criteria in one: the two above are refusals that must run
*before* the arithmetic, where the two the predecessor already carries —
a constant outside `[5, 400]`, a variance that is all noise — refuse after it.
An implementation that adds them beside the others runs them in the wrong
place and passes every case written for the wrong order.

- [ ] 4.1 Write the failing cases first, and note that two of the three do
      not fail the build: a statistic holding no row, and one holding exactly
      one, are recorded as **unmeasured** and the build publishes, dividing
      by `N − 1` of zero nowhere; only a row carrying `matchCount` of 0 fails,
      the ingest never writing one. An earlier draft of this task said all
      three fail, which the requirement it cites contradicts.
- [ ] 4.2 Reach the empty case the way the pipeline reaches it, not with a
      truncated fixture: a run cut short by the daily window leaves the ally
      statistic as staging had it, and step 1.3 already builds that state.
      A case that empties the table by hand proves the guard runs without
      proving the guard is reachable.
- [ ] 4.3 Refuse before every denominator, not after `var_true`. `p_i` and
      `v_i` divide by `n_i`, `var_obs` by `N − 1`, `var_noise` and `k` by `N`
      — all of them before the check that would have caught a non-finite
      result, which is what made this a finding rather than a nicety.

## 5. The ally root in the bundle

Closes `snapshot-export/an-ally-pair-rendered-into-its-own-root`,
`snapshot-export/one-statistic-covered-and-the-other-not`.

- [ ] 5.1 Write the failing cases first (ZOMBIES 16, 17, 18, 21, 22): both
...
- [ ] 5.2 Extend the contract's third level to **both** roots (ZOMBIES 19,
      ...
      each.
- [ ] 5.3 Regenerate `src/fixtures/snapshot.json` and say in the pull request
      which branch each root carries.

## 6. The eighth component
...
- [ ] 6.1 Write the failing cases first (ZOMBIES 23, 24, 25, 26, 27): an
...
- [ ] 6.2 Add `laneSynergy` to `MODEL_CONSTANTS.weights` in `src/types.ts`,
      at 1.0 like the seven beside it. The requirement names
      `MODEL_CONSTANTS.weights.laneSynergy` and nothing else in this change
      creates it, so without this the component multiplies by `undefined` and
      every score becomes `NaN`.
- [ ] 6.3 Sum over the allies already picked, never the entered enemies, and
...
- [ ] 6.4 Do not weight through `laneWeights` (ZOMBIES 30), for the reason
...
- [ ] 6.5 Re-verify the carried *Empty draft components* scenario rather than
...
- [ ] 6.6 Add a case for two bundles alike but for one `laneAllies` value
...
- [ ] 6.7 Re-run Stryker (ZOMBIES 32). Its floor is scoped to `src/model.ts`,
