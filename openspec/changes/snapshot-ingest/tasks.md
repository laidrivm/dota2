...
traceable to the group that closes it.
...
- [ ] 1.3 Write the retry tests: the three delays between four attempts are
      ...
      is attempted exactly once [14]; four consecutive `429`s end the run
      failed [15]. (Req: snapshot-ingest — A request is retried only where
      retrying can succeed)
- [ ] 1.4 Write the failure-classification tests: a `403` carrying
      `text/html` reports an unmet challenge naming the `User-Agent` [11]; a
      `403` carrying JSON reports the key as rejected [12]; a `200` whose
      GraphQL body carries an `errors` array is a failure rather than an empty
      result [17]. (Req: snapshot-ingest — Every request carries both halves
      of the gate / A request is retried only where retrying can succeed)
- [ ] 1.5 Implement the client: the two headers, the key read once at
      ...
      last second, the retry policy, and the classification that reads the
      body before deciding what failed. (Req: snapshot-ingest — Every request
      carries both halves of the gate / A run stays inside the quota the API
      states / A request is retried only where retrying can succeed)
...
- [ ] 2.1 Write the patch tests: a first run against an empty `patches` table
      ...
      current)
- [ ] 2.2 Write the hero-upsert tests: a hero the tables lack is inserted with
      ...
      (Req: hero-reference — A hero is upserted and never removed)
- [ ] 2.3 Implement patch detection against the second source, including the
      major/letter split and the insert-once rule, with a comment naming why
      the statistics API's own version list is not read. (Req: hero-reference
      — Patches are detected from a source that is current)
- [ ] 2.4 Implement the hero upsert with no delete path at all, so removal is
...
- [ ] 3.1 Write the mirroring tests: a first run against an empty directory
      ...
      refetched [54]. (Req: hero-reference — Hero images are mirrored to the
      application's own origin)
- [ ] 3.2 Write the route tests: `/icons/<name>.png` answers `200` with
      `content-type: image/png` and the immutable cache header [48]; an
      unheld name answers `404` with an empty body [49]; a request naming a
      segment outside the mirror directory cannot reach a file outside it
      [50]; a file written after the server started is served without a
      restart [55]. (Req: hero-reference — Hero images are mirrored to the
      application's own origin)
- [ ] 3.3 Implement the mirror: fetch once per hero, skip a name already
      present, and fail the run only where a hero has no file at all. (Req:
...
- [ ] 3.4 Add the `/icons/*` route to `static-routes.ts`, resolved from the
      ...
      that directory while the server is running. (Req: hero-reference — Hero
      images are mirrored to the application's own origin)

## 4. The windows each statistic is pulled over

- [ ] 4.1 Write the meta-window tests: a patch seven whole days old is pulled
      over exactly seven days and each staging row is the sum over them [20];
      a patch less than a day old is pulled over one day rather than zero
      [18]; a hero the source returns no rows for produces no position rows
      rather than rows of zeros [19]; days returned out of order sum to the
      same row as days in order [21]; five position pulls produce one row per
      hero and position with none overwriting another [22]; the day boundary
      is counted in UTC, so a run instant whose local date is a day ahead does
      not add a day [26]. (Req: snapshot-ingest — The meta is pulled by day
      over the current patch's life)
...
- [ ] 4.3 Write the pair-window tests: a patch live for exactly four complete
      ...
      run records which four [24]; one live for two pulls two, the bound being
      a cap and not a quota to fill [25]; a week whose span contains the
      current patch's `detected_at` is attributed to the current patch [31].
      (Req: snapshot-ingest — Pair statistics are pulled per hero over four
      complete weeks)
- [ ] 4.4 Write the pair-response tests: the request asks for every other hero
      ...
      four complete weeks)
- [ ] 4.5 Implement the meta pull: one request per position over the window,
      the game mode and brackets named at one site, summed into staging rows.
      (Req: snapshot-ingest — The meta is pulled by day over the current
      patch's life)
- [ ] 4.6 Implement the pair pull: one request per hero per week over the
      four-week cap, asking for every opponent, summed across the weeks, and
      reporting the weeks covered. (Req: snapshot-ingest — Pair statistics are
      pulled per hero over four complete weeks)
...
- [ ] 5.1 Write the contest tests: a hero whose match count equals the
      ...
      the stated ratio rather than a rounded one [34]; a hero with no picks
      and no bans is 0, with no division by zero on an empty window [35].
      (Req: snapshot-ingest — Contest rate is a share of the window's matches)
- [ ] 5.2 Write the staging tests: two runs over identical responses leave
      identical rows rather than doubled counts [36]; a run failing after the
...
- [ ] 5.3 Implement the contest formula over the counts the meta and ban pulls
      ...
      exact. (Req: snapshot-ingest — Contest rate is a share of the window's
      matches)
- [ ] 5.4 Implement the staging write as a delete-then-insert inside one
      transaction, with the retention that drops anything older than the
      previous patch. (Req: snapshot-ingest — A run leaves staging whole or
      leaves it untouched)
...
- [ ] 6.1 Write the outcome tests: all three steps succeeding exits zero and
      ...
      failing export leaves the previous bundle served and exits non-zero
      [60]; the export runs alone against the newest published snapshot
      without re-ingesting [61]. (Req: snapshot-ingest — The job carries a run
      to one outcome)
- [ ] 6.2 Implement the entry point: ingest, then build, then export, each
      separately callable, returning which step failed and exiting non-zero
