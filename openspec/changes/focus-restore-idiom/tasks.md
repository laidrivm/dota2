...
- [ ] 1.1 Write the e2e case first, in `e2e/board.spec.ts` beside the existing
      *removing a hero moves focus to the entry control that replaces it*: hide
      ...
      `requestAnimationFrame`, so 1.2 is where it earns its place (*Focus survives the removal in a hidden tab*)
- [ ] 1.2 Break-check it: replace the `setTimeout` in
      ...
      nothing, and this is the mistake `CLAUDE.md`'s Code rule was written for (*Focus survives the removal in a hidden tab*)
...
- [ ] 1.5 Switch `src/app/board/pieces.tsx`'s `RemoveButton` to it, keeping the
      ...
      this switch breaks. Its comment about the wait leaves too (*Focus survives the removal in a hidden tab*)
