...
and `bun r` / `bun rm` / `bun uninstall` for `remove`. Three commands mutate
the manifest across eight invocation forms, and the gate covers two of them —
so `bun a preact` and `bun i` reach a manifest write today without ever
prompting, which is the exact boundary the policy exists to hold.
...
- Extend `permissions.ask` to every invocation form that mutates the
  dependency manifest: `bun add`, `bun a`, `bun install`, `bun i`,
  `bun remove`, `bun rm`, `bun r`, `bun uninstall`. Claude Code matches
...
- `.claude/settings.json` — `permissions.ask` grows from two entries to eight.
...
- Six new approval prompts in day-to-day work, all on commands that write
  `package.json`.
