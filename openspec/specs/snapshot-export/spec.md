# snapshot-export Specification

## Purpose

How a published snapshot becomes the bundle at the served URL: the render
that renames every key at that boundary and expands the stored pairs into
full matrices, the flag that marks a settling patch, the runtime assertion
that no payload the client would reject is published, the write-then-rename
that keeps a reader from catching half a bundle, and the URL's two sources
with the validator that makes a return visit cheap.
