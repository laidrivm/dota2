...
The single-page browser application the pick assistant is delivered as:
one screen whose visible blocks are derived from the current session and
snapshot, styled only from the imported design tokens, making no
third-party network requests at runtime, and buildable into a directory of
static files.
