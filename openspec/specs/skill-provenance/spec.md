...
Which shared skills this project's gates depend on, what commit each was
verified against, which are archived, and what the pin can and cannot check.
It exists because `.claude/skills/<name>` points at a working tree in a
separate repository rather than at a commit: whatever is checked out there
when a session starts *is* the gate, an edit there changes how the gates
behave here, and nothing in a diff of this repository shows it.
...
skill's documented contract in that file was last checked against its
...
- **WHEN** a skill's `SKILL.md` no longer matches what `docs/review-toolkit.md`
  says it does
- **THEN** that file is corrected and the commit is advanced in the same
...
skills named in `docs/review-toolkit.md`'s pre-PR sequence together with any
skill a rule under `CLAUDE.md`'s `### Rules` heading depends on —
`playwright-cli` is a gate by such a rule and not by the sequence. The test
SHALL require exactly one active row per member of that set, SHALL reject a
duplicate row and an active row for a skill in neither source, and SHALL
require the archived list to be disjoint from the active one.
...
`.claude/skills/`: the entries this capability is about are symlinks pointing
...
#### Scenario: A skill is named outside the rules

- **WHEN** `CLAUDE.md` names a skill in prose outside its `### Rules` section
- **THEN** the active set does not gain it, because prose that names a skill
