# snapshot-ingest Specification

## Purpose

How staging is filled: the gate every request to the statistics API must
clear, what a response means before its status is read, the quota a run
paces itself inside, the retries that can succeed and the ones that cannot,
the windows the meta and pair pulls cover, how contest rate is derived, what
a run leaves behind when it fails part-way, the one entry point that carries
a run to a single outcome, and what that run records about the days it
covered.
