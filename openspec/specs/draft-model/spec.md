...
The pure, deterministic client-side computation at the heart of the pick
assistant: `computeModel(bundle, session)` maps a published snapshot bundle
plus the current draft session to enemy-role probabilities, per-role
suggestion scores, and (at a full draft) a win probability — recomputed
synchronously on every session change, with no persistence or I/O.
