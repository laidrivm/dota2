...
The modal overlay that puts a hero into the draft: what opens it and for which
position, how its search matches names and aliases, what its grid shows for
heroes already on the board, how it is driven from the keyboard, and why none
of its state outlives the pick.
